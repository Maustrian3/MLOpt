...
# Input search ranges
num_golfers = range(8, 33)  # Because the classic SGP includes 32 golfers, we extend the range to include 32
num_weeks = range(2, 11)    # Because the required original family uses weeks 6..10, we extend weeks to include 10
...
# we force-add these instances even if random sampling misses them
...
# we de-duplicate to keep one row per instance triple
final_instances = list(set(final_instances))

# Because the CSV is easier to inspect/debug when ordered, we sort by (weeks, golfers, groups)
